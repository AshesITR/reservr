...
Interval censored observations were described in \citet[Case 2]{GroeneboomWellner1992}: let $(Y, M_0, V_0)$ denote a random vector on $\mathbb{R}^3$, where $Y$ is the variable of interest, with c.d.f. $F_\theta$ depending on a parameter $\theta \in \Theta$.
The pair $(M_0, V_0)$ shall satisfy $M_0 \le V_0$ with probability one.
Further, assume $Y$ to be independent of $(M_0, V_0)$.
Let
\begin{align*}
...
\begin{align*}
  ...
  \end{cases}.
\end{align*}
Therefor, $(M, V)$ contains all information necessary to compute the conditional likelihood of $\theta$ for $D$ given $(M_0, V_0)$.

\begin{align}
\ell(\theta | M = m, V = v) & := \log F_\theta((m, v]). \label{eq:llik-cens}
\end{align}

A sample of _non-informatively interval censored observations_ from $(M, V)$ consists of independent observations $(m_i, v_i)$.
Estimation of $\theta$ can can be performed using conditional maximum likelihood estimation with \eqref{eq:llik-cens}.

The random truncation problem is formulated as follows in \citet{Rosenstock2022}: let $(Y, L, U)$ denote a random vector on $\mathbb{R}^3$, where $Y$ is the variable of interest that is supposed to have a density $f_\theta$ with respect to some dominating sigma-finite measure $\mu$.
...
A sample of _randomly truncated observations_ from $(Y, L, U)$ consists of independent observations $(y_i, l_i, u_i)$ that we only happen to see if $l_i < y_i \le u_i$.
...
Truncation and censoring can occur at the same time, which can be described by a random vector $(Y, L, U, M_0, V_0)$, where $L \le M_0 \le V_0 \le U$ with probability one.
We let $D = \mathbf{1}(Y > M_0) + \mathbf{1}(Y > V_0)$, such that

\begin{align}
  \Prob(D = d | L, U, M_0, V_0, L < Y \le U) & = \begin{cases}
    \Prob(Y \le M_0 | M_0, V_0, L < Y \le U) & d = 0 \\
    \Prob(M_0 < Y \le V_0 | M_0, V_0, L < Y \le U) & d = 1 \\
    \Prob(V_0 < Y | M_0, V_0, L < Y \le U) & d = 2
  \end{cases} \nonumber \\
  & = \frac{1}{F_\theta((L, U])} \begin{cases}
    ...
    F_\theta((V_0, U]) & d = 2
  \end{cases} \\
  & = \frac{F_\theta((M, V])}{F_\theta((L, U])}, \label{eq:lik-trunc-cens0}
\end{align}
where $(M, V)$ is defined as above.

A sample of _randomly truncated non-informatively interval censored observations_ from $(Y, L, U, M, V)$ consists of independent observations $(l_i, u_i, m_i, v_i)$ that we only happen to see if $l_i < y_i \le u_i$.
The conditional log-likelihood for $\theta$ given $(L, U, M, V)$ is
\begin{align}
...
Note that this subsumes untruncated observations by allowing $l = -\infty$ and $u = \infty$.
