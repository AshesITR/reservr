...
\textcolor{orange}{[AR: Hier 'annotated' sample mit Prädiktoren definieren]}
...
Suggested by standard maximum (conditional) likelihood approaches for truncated \citep{DoerreEmura2019} and censored observations \citep{Zhang2005, Sun2005}, we suggest to estimate $\theta$ by maximizing the objective function
\textcolor{orange}{[AR: Sun2005 bitte in bibliography.bib ergänzen, ich finde das nicht.]}
<!-- -->
\begin{align}
...
\textcolor{orange}{[AR: Hier distributional regression beschreiben (aktuell siehe weiter unten)]}
...
The package serves two main goals: fitting distributions to randomly truncated non-informatively \textcolor{red}{[Das Stcihwort non-informative wurde zuvor bislang nicht definiert]}\textcolor{orange}{[AR: Können wir auch weglassen, da wir interval censoring geeignet definiert haben.]} interval censored data and performing (deep) distributional regression with randomly truncated non-informatively interval censored data.
...
* use additional functions supporting parameter estimation procedures such as computing support and discreteness \textcolor{red}{[AB:Was heißt 'Computing discreteness'?]}\textcolor{orange}{[AR: Siehe \texttt{is\_discrete\_at()} unten. Bestimmt, ob die Familie an einer bestimmten stelle eine Punktmasse liegt.]},
...
6. Uniform distribution families with free lower bound `min` or upper bound `max` (estimated by the minimum, for `min`, and the maximum, for `max`, over the sample)

\textcolor{red}{[AB: bei 5 und 6 verstehe ich nicht genau, was getan wird.]}
\textcolor{orange}{[AR: 5.: $(m, v, l, u)$ werden durch $(m-b)/a$ etc. ersetzt und dann die zugrundeliegende Familie angepasst. 6.: $\min(m)$ wird as Schätzer für den linken Randpunkt genommen und $\max(v)$ für den Rechten - jeweils falls diese frei sind.]}
...
\textcolor{red}{[AB: Was genau macht flattenparams? Evtl. hilft eiom Beispiel an geeigneter Stelle.]}
\textcolor{orange}{[AR: Das macht die Listenstruktur platt. Der \texttt{str(fit(...))} call zwei zeilen weiter unten zeigt unter \texttt{params} die klobige Version.]}
...
This introduces two parameters to the function family $\mathcal{G}$ and implies the functional relationship $\mu = g(x) := \theta_1 \cdot x + \theta_0$. \textcolor{red}{[AB: Ich verstehe nicht, wieso hier eine lineare Funktion auftaucht; sollte nciht $g$ ein ggf. kompliziertes neuronales Netz sein?]}
\textcolor{orange}{[AR: Die Flexibilität besteht, aber das Beispiel hier ist ja lineare regression.]}
