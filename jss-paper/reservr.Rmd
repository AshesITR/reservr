...
More recently, deep distribution regression allows for flexible specification of individual outcome distribution parameters.
...
It is split into definition of samples $\mathfrak{I}$ (Section&nbsp;\ref{trunc-obs}), definition of distribution families (Section&nbsp;\ref{distributions}), estimation of distribution parameters (Section&nbsp;\ref{fit-dist}) and distribution regression using \pkg{tensorflow} (Section&nbsp;\ref{tensorflow}).
A conclusion is given in Section&nbsp;\ref{conclusion}.

# Usage of \pkg{reservr} {short-title="Usage of reservr" #pkg-overview}

The package serves two main goals: fitting distributions to randomly truncated interval censored data and performing (deep) distribution regression with randomly truncated interval censored data.
...
TODO describe `Distribution` class, parameters and methods
describe erlang mixtures, blended distribution, mixture distribution
...
## Distribution regression using \pkg{tensorflow} integration {short-title="Distribution regression using tensorflow integration" #tensorflow}
