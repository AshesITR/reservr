...
The pair $(M_0, V_0)$ shall satisfy $M_0 \le V_0$.
...
\begin{align}
  ...
  \end{cases}. \nonumber
\end{align}
This leads to the conditional log-likelihood

\[
\ell(\theta | M_0, V_0) = \log F_\theta([M, V]).
\]
...
x_upper <- x
x_upper[is_censored] <- x_lower[is_censored] + runif(sum(is_censored), min = 0, max = 1.0)
...
| `dist_trunc(dist, min, max)`                     | A truncated distribution, $\{X \mid \text{min} \le X \le \text{max}\}$ for $X \in \mathcal{F}$ from the underlying family |
 ...
 * `get_type()` returns one of `"continuous"`, `"discrete"`, or `"mixed"` depending on whether the distribution has a density with respect to the Lebesgue measure, the counting measure, or the sum of the Lebesgue measure with one or many point measures.
...
We presented \pkg{reservr}, a package that supports distribution estimation and distribution regression using \proglang{R}.
