...
Most statistical analyses are concerned with modelling and estimation of the distribution of some measured variable of interest, called the outcome, possibly conditional on the value of one or several endogenous variables, called predictors.
...
Classical regression, such as via generalized linear models, is usually concerned with the influence of endogenous variables on the mean of the outcome, i.e., $\E(Y|X) = f(X)$ for some random variable $Y$ and vector of associated endogenous variables $X$ paired with an assumption linking the other parameters of the outcome distribution to its mean.
Some models also allow specification of additional parameters of the distribution of the outcome variable, such as Generalized Additive Models for Location Scale and Shape \citep{GAMLSS}, which also allows for the shape parameter to depend on endogenous variables.
...
An example implementation of the latter for complete data can be found in \citet{deepregression}.

Usual statistical methods require complete data, that is full
information on all observations of interest \((X, Y)\) where \(Y\) is
some outcome of interest and \(X\) representing corresponding
predictors. In addition to this usual scenario, we study three
observation schemes that do not provide all observations. Data with
\emph{interval censoring} applied to the outcome \(Y\) only consists of
lower and upper bounds for \(Y\) instead of the actual value. Truncated
data misses observations for which the outcome \(Y\) falls out of a
certain lower and upper truncation bound. We consider the case of
\emph{random truncation}, where these truncation bounds are also random
variables that vary for each observation. Finally, we consider a
combination of the two, \emph{randomly truncated interval censoring}.
More precisely, instead of observing the real-valued target variable
\(Y\) (with \(\mu\)-density \(f_\theta\) and c.d.f.~\(F_\theta\)), we
observe the vector \((M,V,L,U)\), which satisfies
\(L \le M \le V \le U\) and \(L<U\). Its coordinates have the following
interpretation: the last two coordinates, which satisfy
\(- \infty \le L < U \le \infty\), encode truncation: we only happen to
observe \((M,V,L,U)\) if \(L < Y \le U\); in particular, a non-truncated
observations means that \(L=-\infty\) and \(U=\infty\). The first two
coordinates, which satisfy \(L \le M \le V \le U\) and which may be
\(\mp \infty\), encode censoring: the observation \((M,V)=(m,v)\) means
that the target variable \(Y\) satisfies \(Y \in (m,v]\) if \(m<v\) and
\(Y=m\) if \(m=v\); the latter corresponds to an uncensored observation
of \(Y\).

Suppose we observe an independent sample
\(\mathcal J = \{(m_i,v_i,l_i, u_i): i=1, \dots, n \}\) of
\((M,V,L,U)\). Suggested by standard maximum (conditional) likelihood
approaches for truncated \citep{???} and censored observations
\citep{Sun2005}, we suggest to estimate \(\theta\) by maximizing the
objective function \begin{align}
\ell(\theta) = \sum_{(m,v,l, u) \in \mathcal J} \Big\{ \log f_\theta(m)  1(m=v) + \log F_\theta((m,v]) 1 (m<v) \Big\} - \log F_\theta((l,u]),
\label{eq:cml-likelihood-nw}
\end{align} where we use the notation
\(F_\theta((l,u])=F_\theta(u) - F_\theta(l)\). A detailed motivation for
this approach under suitable conditions ensuring that the censoring is non-informative  
is given in Section xxxx below. For later purposes, it is
helpful to attach a weight \(w_i\) to each observation
\((m_i, v_i, l_i, u_i)\). Denoting the resulting sample by
\(\mathfrak{I} = \{(m_i, v_i, l_i, u_i, w_i)\}\), we aim at maximizing
the weighted sum of the (conditional) log-likelihoods \begin{align}
\ell(\theta) = \sum_{(m,v,l, u,w) \in \mathcal J} w \cdot \Big[ \Big\{ \log f_\theta(m) 1(m=v) + \log F_\theta([m,v]) 1 (m<v) \Big\} - \log F_\theta((l,u]) \Big].
\label{eq:cml-likelihood2}
\end{align}

A practical example of random truncation and interval censorship arises
in the presence of inaccurate accident date specifications when
reporting claims in general insurance. The target variable $Y$ is the reporting delay 
of an accident happening at accident time $T_0$, which is hence reported to the insurer at calendar time $Y+T_0$.
The truncation bounds \((L, U)\)
for $Y$ will be equal to \((0, \tau - T_0)\) with \(\tau\)  the current calendar time. Censoring bounds
could, for example, be
\((M, V) = (Y+T_0 - \lceil T_0 \rceil, Y + T_0 - \lfloor T_0 \rfloor)\)
representing an inaccurate observation of \(T_0\) at reporting time
\(Y+T_0\). Technically, \(U\) is not (fully) observable in this situation,
but can be approximated by \(\tau - \lfloor T_0 \rfloor\). It should be
noted that the type of censoring described in this example is not
non-informative because \((M, V)\) is not independent of \(Y\).  \textcolor{red}{[AB: Ein Beispiel, bei dem beide Unabhängigkeits-Annahmen sinnvoll sind, wäre super.]}



\paragraph{Motivation of \eqref{eq:cml-likelihood-nw}.}

It is instructive to start by considering an untruncated, censored
observation where \(l=-\infty, u=+\infty\) and \(m<v\). The only
information we obtain from the observation \((m,v,l,u)\) is then that
\(T \in (m,v]\). For deriving the relevant likelihood contribution, we
may follow the stochastic approach to interval censored observations
described in \citet[Case 2]{GroeneboomWellner1992}: let \((C_1,C_2)\) 
\textcolor{red}{[AB: habe die Bezeichnung geändert, da die Möglichkeit $V=M_0$ von zuvor von der Notation verwirrend sein kann.]}
denote a random vector in \(\mathbb R^2\) that is independent of the
target variable \(Y\) and which satisfies \(\Prob(C_1 < C_2)=1\). Let
\begin{align*}
  D & := \mathbf{1}(Y > C_1) + \mathbf{1}(Y > C_2),
\end{align*} and define new random variables \((M, V) = f(Y,C_1,C_2)\)
by \begin{align*}
  (M, V) & := \begin{cases}
    (-\infty, C_1), & D = 0, \\
    (C_1, C_2), & D = 1, \\
    (C_2, \infty), & D = 2.
 \end{cases}
\end{align*} Note that \(D\) can be reconstructed from \((M, V)\): we have $D=0$ if $M=-\infty$, $D=1$ if $-\infty<M<V<\infty$ and $D=2$ if $V=\infty$.  
It is instructive to proceed with the case where
\((C_1,C_2)\) and hence \((M,V)\) is discrete. Then, for
\((m,v) \in \mathrm{supp}(M,V) \cap \mathbb R^2\), we have
\begin{align*}
\Prob(M=m, V=v)
&=
\Prob(M=m, V=v, D=1) \\
...
\end{align*} Likewise, for
\((m,v) \in \mathrm{supp}(M,V) \cap ( \{-\infty\} \times \mathbb R)\),
we obtain \begin{align*}
\Prob(M=-\infty, V=v)
&=
\Prob(M=-\infty, V=v, D=0) \\
&= 
\Prob(C_1=v,  Y \le v) \\
&=
F_\theta((-\infty, v]) \cdot \Prob(C_1=v) 
\end{align*} and finally, for
\((m,v) \in \mathrm{supp}(M,V) \cap (\mathbb R \times \{\infty\} )\),
\begin{align*}
\Prob(M=m, V=\infty)
&=
\Prob(M=m, V=\infty, D=2) \\
&= 
\Prob(C_2=m,  Y > m) \\
&=
F_\theta((m,\infty])  \cdot \Prob(C_2=m).
\end{align*}
\textcolor{red}{[AB: Sanity check: a quick calculation shows that $\sum_{(m,v) \in \mathrm{supp}(M,V)} \Prob(M=m, V=v) = 1$.]}
If we assume that the distribution of the censoring variable
\((C_1,C_2)\) is non-informative, i.e., its distribution does not depend
on \(\theta\), the likelihood of observing \((M,V)=(m,v)\) is equal to
\(F_\theta((m,v])\), up to a factor that does not depend on \(\theta\).
A similar argumentation can be used in the non-discrete case. Overall, noting that that $F_\infty((-\infty,\infty])=1$,
we have motivated the likelihood contribution \(F_\theta((m,v])\cdot 1(m < v)\) for a censored, untruncated observation in
\eqref{eq:cml-likelihood-nw}.

Next, consider an uncensored, truncated observation \((m,v,l,u)\) where
\(y=m=v\); we may hence identify such an observation with \((y,l,u)\).
We may then proceed as in \citet{Rosenstock2022} and assume that
\((L, U)\) is independent of \(Y\) and satisfies \(L \le U\), with \(L\)
possibly equal to \(-\infty\) and \(U\) possibly equal to \(+\infty\).
Further, \((L, U)\) shall have a density \(f_{(L, U)}\) with respect to
some dominating sigma-finite measure \(\nu\). Truncation means that we
only happen to observe \((Y,L,U)\) if \(L < Y \le U\). As a consequence,
any observed value with \(M=V\) can be regarded as being drawn from the
\((\mu \otimes \nu)\)-density \begin{align}
  f_{(Y, L, U) | L < Y \le U}(y, l, u) = \frac{f_{(L, U)}(l, u) f_\theta(y)}{\Prob(L < Y \le U)} \mathbf{1}(l < y \le u). \label{eq:trunc-dens}
\end{align} Subsequently, we write \((Y^{(t)}, L^{(t)}, U^{(t)})\) for a
random vector following the above density, i.e., \[
  f_{(Y^{(t)}, L^{(t)}, U^{(t)})}(y, l, u) = f_{(Y, L, U) | L < Y \le U}(y, l, u).
\] Conditioning this density on \((L^{(t)}, U^{(t)})=(l,u)\), we arrive
at an expression that does not involve the nuisance density
\(f_{(L,U)}\): \begin{align*}
...
\end{align*} Overall, we arrive at the (conditional) log-likelihood
contribution \(\log f_\theta(y) - \log F_\theta((l, u])\) for a noncensored, truncated observation in \eqref{eq:cml-likelihood-nw}.


Finally, truncation and censoring can occur at the same time, i.e., we
have \(l \le m<v \le u\) with either \(l \ne - \infty\) or
\(u\ne \infty\). In accordance with the previous two cases, we make the assumption that $Y, (C_1,C_2)$ and $(L,U)$ are mutually independent  and satisfy $C_1<C_2$ and $L<U$. Define
\[
D = \mathbf{1}(Y > C_1) + \mathbf{1}(Y > C_2)
\]
and
...
\begin{align*}
\Prob(M=m, V=v | L=l, U=u, L < Y \le U) 
&=
\frac{\Prob(C_1=m, C_2=v, Y \in (m,v], L=l, U=u) }{ \Prob(L=l, U=u, l < Y \le u)} \\
&=
\frac{\Prob(C_1=m, C_2=v) F_\theta((m,v])}{F_\theta((l,u])}
\end{align*}
by the independence assumption. The factor not in front does not depend on $\theta$ and is irrelevant for the (conditional) likelihood contribution. Likewise, in case $l=m<v<u$, we have
\begin{align*}
\Prob(M=l, V=v | L=l, U=u, L < Y \le U) 
&=
\frac{\Prob(M=l, V=v,  L=l, U=u, l< Y \le u) }{ \Prob(L=l, U=u, l< Y \le u)}.
\end{align*}
Bv definition of $(M,V)$, the event in the numerator  is the disjoint union of the following two sets:
\begin{align*}
&\{ D=0, C_1=v, L=l,U=u , l< Y \le u\} 
= \{ C_1=v, L=l, U=u, Y \in (l,v]\} \\
...
\Prob(M=l, V=v | L=l, U=u, L < Y \le U) 
=
\{ \Prob(C_1=v) + \Prob(C_1 \le l, C_2=v) \} \frac{F_\theta((l,v])}{ F_\theta((l,u])}.
\]
...
\textcolor{red}{[AB: In der Tat: Dritter Fall: $l<m<v=u$.
\begin{align*}
\Prob(M=m, V=u | L=l, U=u, L < Y \le U) 
&=
\frac{\Prob(M=m, V=u,  L=l, U=u, L< Y \le U) }{ \Prob(L=l, U=u, L< Y \le U)}.
\end{align*}
Das Ereignis im Zähler ist die disjunkte Vereinigung der beiden Ereignisse 
\begin{align*}
&\{ D=2, C_2=m, L=l,U=u , L< Y \le U\} 
= \{ C_2=m, L=l, U=u, Y \in (m,u]\} \\
&\{ D=1,C_1=m, C_2\ge u, L=l,U=u , L< Y \le U\} = \{C_1 =m, C_2\ge u, L=l, U=u, Y \in (m,u]\},
\end{align*}
so dass
\begin{align*}
\Prob(M=m, V=u | L=l, U=u, L < Y \le U) 
=
\frac{\{ \Prob(C_2=m) + \Prob(C_1 = m, C_2\ge u) \} F_\theta((m,u])}{ F_\theta((l,u])}.
\end{align*}
Zuletzt betrachte $l=m<v=u$. Dann:
\begin{align*}
\Prob(M=l, V=u | L=l, U=u, L < Y \le U) 
&=
\frac{\Prob(M=l, V=u,  L=l, U=u, L< Y \le U) }{ \Prob(L=l, U=u, L< Y \le U)}.
\end{align*}
Das Ereignis im Zähler ist die disjunkte Vereinigung der drei Ereignisse 
\begin{align*}
&\{ D=0, C_1\ge u, L=l,U=u , L< Y \le U\} 
= \{ C_1 \ge u, L=l, U=u, Y \in (l,u]\} \\
&\{ D=1, C_1\le l, C_2\ge u, L=l,U=u , L< Y \le U\} = \{ C_1\le l, C_2\ge u, L=l, U=u, Y \in (l,u]\} \\
&\{ D=2, C_2\le l, L=l,U=u , L< Y \le U\} = \{ C_2\le l, L=l, U=u, Y \in (l,u]\},
\end{align*}
so dass 
\begin{align*}
\Prob(M=l, V=u | L=l, U=u, L < Y \le U) 
&=
\frac{\{ \Prob(C_1 \ge u) + \Prob(C_1 \le l, C_2\ge u) + \Prob(C_2 \le l) \} F_\theta((l,u])}{ F_\theta((l,u])}\\
&=
\Prob(C_1 \ge u) + \Prob(C_1 \le l, C_2\ge u) + \Prob(C_2 \le l)
\end{align*}
Der Ausdruck hängt nicht von $\theta$ ab; kann also bei der Likelihood ignoriert werden. Das ist anschaulich plausibel: wir können $Y$ nur zwischen den Trunkierungsgrenzen beobachten, und die Zensierung $M=L, V=U$ ergibt uns über den Wert von $Y$ keine Zusatzinformation.]}
...
The package serves two main goals: fitting distributions to randomly truncated non-informatively interval censored data and performing (deep) distributional regression with randomly truncated non-informatively interval censored data.
Four main components are integrated with each other to facilitate the analysis goals

1. Methods for describing a randomly truncated non-informatively interval censored sample $\mathfrak{I}$.
2. Methods for specifying distribution families $\mathcal{F}$ to be fitted.
3. Methods for estimating distribution parameters $\theta$ given a sample $\mathfrak{I}$ and a parametrized family $\mathcal{F} = \{F_\theta | \theta \in \Theta\}$.
4. Methods for regression of distribution parameters given a sample $\mathfrak{I}$ with associated predictors $X \in \mathfrak{X}$, a parametrized family $\mathcal{F}$ and a general \pkg{tensorflow} network $\mathcal{G} : \mathfrak{X} \to \Theta$ that processes $X$ to estimate $Y | X = F_{g(X)}$.
 ...
 * `xmin`: Lower interval censoring bound ($M$ in \ref{introduction}) for the observation. If the observation is not censored, `xmin` is equal to `x`.
 * `xmax`: Upper interval censoring bound ($V$ in \ref{introduction}) for the observation. If the observation is not censored, `xmax` is equal to `x`.
 * `tmin`: Lower truncation bound ($L$ in \ref{introduction}). Only observations with $x \ge t_{\text{min}}$ are observed. Can be $-\infty$ to indicate no lower truncation.
 * `tmax`: Upper truncation bound ($U$ in \ref{introduction}). Only observations with $x \le t_{\text{max}}$ are observed. Can be $\infty$ to indicate no upper truncation.
 * `w`: The weight associated with the observation. Defaults to $1$.

Note that, unlike \ref{introduction}, the lower bounds of intervals in `trunc_obs` are included.
For continuous random variables, the formulas are equivalent to the half-open formulation (since $\Prob(Y \in \{M, V\} | M, V) = 0$).
For discrete random variables, $M$ and $L$ may have to be appropriately shifted.
...
* use basic distribution functions for random number generation, computation of density, cumulative density, hazard and quantile,
...
| `dist_translate(dist, offset, multiplier)`       | An affine transformation family $\{a \cdot F + b\}$ for $F \in \mathcal{F}$ from the underlying family                                  |
| `dist_trunc(dist, min, max)`                     | A truncated distribution family, $\{F\restr_{\text{min} \le \cdot \le \text{max}}\}$ for $F \in \mathcal{F}$ from the underlying family |
...
The basic distribution functions (density, probability, hazard, quantile, random number generation) are provided by each distribution family.
 ...
 * `probability(q, lower.tail = TRUE, log.p = FALSE, with_params = list()` computes the (log-)cumulative density or (log-)survival.
 * `hazard(x, log = FALSE. with_params = list())` computes the (log-)hazard.
...
This can be populated by passing `trace = TRUE` to `fit()` and will record parameters after all ECME steps in the ECME-based estimation algorithms \citet[Algorithms 1 and 2]{Rosenstock2022} and \citet[Algorithm S.2]{Rosenstock2022supp}.
...
Compared to Equation \ref{eq:cml-likelihood}, the global parameter $\theta$ is replaced by the regression function $g$ evaluated at the associated covariates.

The maximization problem is delegated to \pkg{tensorflow}, which supplies ample stochastic optimization algorithms.
Functions in \pkg{reservr} are necessary to create a suitable output layer for \pkg{tensorflow} that maps onto $\Theta$ and to provide an implementation of the (negative) log-likelihood \eqref{eq:cml-regression-likelihood} as a loss function.
