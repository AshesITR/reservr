...
Usual statistical methods require complete data, that is full information on all observations of interest $(X, Y)$ where $Y$ is some outcome of interest and $X$ representing corresponding predictors.
In addition to this usual scenario, we study three observation schemes that do not provide all observations.
...
More precisely, instead of observing the real-valued target variable $Y$ (with $\mu$-density $f_\theta$ and c.d.f. $F_\theta$), we observe the vector $(M,V,L,U)$, which satisfies
...
Suggested by standard maximum (conditional) likelihood approaches for truncated \citep{???} and censored observations \citep{Sun2005}, we suggest to estimate $\theta$ by maximizing the objective function 
\begin{align}
...
A detailed motivation for this approach under suitable conditions ensuring that the censoring is non-informative is given in Section xxxx below.
