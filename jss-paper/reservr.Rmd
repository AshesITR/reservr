...
abstract: >
  ...
  This article presents the \proglang{R} package \pkg{reservr}, which implements distribution parameter estimation and distributional regression for randomly truncated and interval censored data based on (conditional) maximum likelihood.
  ...
  Distributional regression is supported via an interface to the \proglang{R} package \pkg{tensorflow} to build neural network models for distributional regression of censored and randomly truncated outcomes with arbitrary distribution families.
...
keywords:
  formatted: [distribution fitting, random truncation, interval censoring, distributional regression, deep learning, "\\proglang{R}"]
  plain:     [distribution fitting, random truncation, interval censoring, distributional regression, deep learning, R]
preamble: >
  \usepackage{amsmath}
  \usepackage{amssymb}
...
Suggested by standard maximum (conditional) likelihood approaches for truncated \citep{DoerreEmura2019} and censored observations \citep{Sun2006}, we suggest to estimate $\theta$ by maximizing the objective function
<!-- -->
\begin{align}
...
Compared to Equation \eqref{eq:cml-likelihood}, the global parameter $\theta$ is replaced by the regression function $g$ evaluated at the associated predictors $x$.

## Motivation of Equation \eqref{eq:cml-likelihood-nw}. {short-title="Motivation of the conditional likelihood" #motivation-cml-likelihood}
...
Among these are \pkg{MASS} \citep{MASS}, \pkg{fitdistrplus} \citep{fitdistrplus}, \pkg{survival} \citep{survival}, \pkg{flexsurv} \citep{flexsurv}.
...
Distributional regression packages, such as \pkg{gamlss} \citep{GAMLSS} and \pkg{deepregression} \citep{deepregression} currently do not support interval censoring or random truncation.
...
|----------------------|----------------|-----------|-------------------|----------------|
| \pkg{MASS}           | no             | no        | no                | classic        |
| \pkg{fitdistrplus}   | only integer   | supported | no                | no             |
| \pkg{survival}       | supported      | supported | no                | classic        |
| \pkg{flexsurv}       | supported      | supported | no                | classic        |
| \pkg{gamlss}         | supported      | no        | no                | distributional |
| \pkg{deepregression} | supported      | no        | no                | distributional |
| \pkg{reservr}        | supported      | supported | supported         | distributional |
...
4. Methods for regression of distribution parameters given a regression sample $\mathfrak{I}_{\text{reg}}$, a parametrized family $\mathcal{F}$ and a general \pkg{tensorflow} network $\mathcal{G} : \mathfrak{X} \to \Theta$ that processes $X$ to estimate the conditional distribution of $Y | X = x$ by $F_{g(x)}$ with $g \in \mathcal G$.
...
Such an analysis was performed using \pkg{reservr} in \citet{Rosenstock2022,Rosenstock2023}.
...
Details on working with \pkg{tensorflow} can be found in Section \ref{tensorflow}.
...
The latter distributions can be obtained in \pkg{reservr} by setting the probability weights of the blended distribution to $p = (1, 0)$ and $p = (0, 1)$ respectively.
...
distb_data <- tibble::tibble(
  ...
  transformation = forcats::fct_inorder(rep(c("original", "truncated", "blended"), each = 2L))
)
...
The maximization problem \eqref{eq:cml-regression-likelihood} is delegated to \pkg{tensorflow}, which supplies ample stochastic optimization algorithms.
...
* Compiles the loss for `dist` defined by \eqref{eq:cml-regression-likelihood} as $l(g) = -\tfrac{1}{\#(\mathfrak{I}_{\mathrm{reg}})} \ell(g|\mathfrak{I}_{\mathrm{reg}})$, optionally disabling `censoring` or `truncation` for efficiency.
...
\textcolor{red}{Das Bild beinhaltet nur einen Punkt..?}
...
\textcolor{red}{Es werden warnings angezeigt (Removed 26 rows containing missing values). Die Linie im folgenden Bild ist konstant.}
