...
\begin{align*}
  \mathop{\mathrm{BDEGP}}(n, m, \kappa, \varepsilon) := & \mathop{\mathrm{Mixture}}( \\
    ...
    & \qquad \mathop{\mathrm{Blended}}( \\
      ...
      & \qquad\qquad \mathop{\mathrm{GPD}}(u = \kappa, \sigma \in (0, \infty), \xi \in [0, 1]); \\
...
* $\delta_k$ is the dirac distribution with $P(X = k) = 1$
* $\mathrm{ErlangMixture}(m)$ is the family of Erlang mixture distributions with $m$ components
* $\mathrm{GPD}$ is the family of generalized Pareto distributions - constrained to having tail index $\xi \in [0, 1]$
* $\mathrm{Mixture}(\ldots)$ is the family of mixture distributions of the $n+1$ components.
...
2. A flexible family of distributions for its body (note that Erlang mixtures are dense in the space of distributions on $(0, \infty)$)
